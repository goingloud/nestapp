...
%   the input. fsEff is the working rate after any decimation (== fs when none
...
%     1. Bandwidth - integer-decimate (anti-aliased) so the working rate sits
%        just above 2*PSD_FMAX_HZ. EEG QA only needs the neural band plus the
%        artifact tells just above it (50/60 Hz line + first harmonics, onset of
%        broadband muscle); nothing above a few hundred Hz is informative. This
%        also shrinks the data, so memory stays small.
...
%        low-variance, consistent estimate (Welch 1967). 1 Hz resolves every QA
%        feature (alpha ~2-3 Hz, line spikes, broadband muscle).
%
%   It degrades gracefully on any input: fs <= 2*PSD_FMAX_HZ -> no decimation;
%   a short signal -> the window clamps to its length.
%
%   See also: renderQualityFigure, computeICAQualityMetrics, pwelch

% ---- QA spectrum constants (edit here to change every QA PSD) ----------------
PSD_FMAX_HZ = 300;   % top of the QA frequency axis (Hz). Covers the neural band
                     % plus the artifact tells just above it (50/60 Hz line and
                     % its first few harmonics, onset of broadband muscle).
                     % Nothing above a few hundred Hz is informative for QA.
PSD_RES_HZ  = 1;     % Welch frequency resolution = 1 / window-duration (Hz).
                     % 1 Hz resolves every QA feature while keeping the window
                     % short, which maximises averaging (and so accuracy) on a
                     % long recording. Lower it for sharper features at the cost
                     % of fewer averages (more variance).
% -----------------------------------------------------------------------------

    x = double(x(:));
    q = max(1, floor(fs / (2 * PSD_FMAX_HZ)));   % integer decimation factor
    if q > 1
