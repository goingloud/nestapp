...
function ax = sharedColorbar(parent, axesFcn, rect, clim)
% SHAREDCOLORBAR  One colour bar for a grid of maps drawn on one scale.
%
%   ax = SHAREDCOLORBAR(parent, axesFcn, rect, clim) makes an invisible axes at
%   rect carrying the diverging colormap and clim, and puts a colour bar on it.
%   Returns the axes so the caller can delete it.
%
...
CB_LABEL = 'uV';    % every scale this bar describes is microvolts
