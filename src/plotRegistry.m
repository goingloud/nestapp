...
% wave without one invites reading noise as an effect. The LEVEL is still a
% choice, and one that has to be stated - see levelParam.
p.params   = [levelParam(), ...
...
p.params   = [ ...
    makeParam('measure', 'Measure', '', 'mean | peak | area', ...
       ['Which per-window number to plot. "mean" is the average amplitude ' ...
        'over the window; "peak" the largest deflection of the window''s ' ...
        'polarity, meaningful for a signed TEP; "area" the integral, the ' ...
        'natural one for GMFP and LMFP.'], ...
...
function p = levelParam()
% The confidence level of the interval, wherever one is drawn.
%
% A DRAW option, not a compute one: everything expensive - the collapse to one
% curve per subject, the standard error, the paired normalisation - is already
% inside the estimate's .sem, and the bounds are mean -/+ t*sem. So any level
% costs one betaincinv, which is also why this accepts an arbitrary number
% rather than offering a menu of three. See intervalAtLevel.
p = makeParam('level', 'Confidence level', '', '0 to 1', ...
    ['The interval''s coverage, as a fraction - 0.95 for a 95% interval, ' ...
     '0.9 for a narrower one. Whatever is chosen is stated on the plot and ' ...
     'in the exported figure''s footer, so a band and its label cannot ' ...
     'disagree.'], ...
    'type', 'scalar', 'placeholder', '(0.95)');
end
